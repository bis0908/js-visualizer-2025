...
export function QueuePanel({ title, type, items, className }: QueuePanelProps) {
  ...
  return (
    <div className={cn("h-full bg-card border rounded-lg overflow-hidden flex flex-col", className)}>
      {/* Panel header */}
      <div className="h-12 flex items-center justify-between px-4 border-b">
        <div className="flex items-center gap-2">
          <h2 className="text-base font-semibold">{title}</h2>
          {type === "microtask" && (
            <Zap className="w-4 h-4 text-purple-500" aria-label="High priority" />
          )}
        </div>
      ...
      <div className="flex-1 overflow-y-auto p-4">
        {items.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-32 text-center">
            <div className={cn("w-16 h-16 mb-3 rounded-full flex items-center justify-center", colors.bg)}>
          ...
          <div className="space-y-2">
            {items.map((item, index) => (
              ...
              >
                ...
                {/* Priority indicator */}
                <div className={cn("w-1 h-full rounded-full absolute right-0 top-0", colors.priority)} />
              </div>
            ))}
          </div>
        )}
